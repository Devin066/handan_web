...
async function readConfiguration(ctx: Context, companyUuid: string) {
  const rows = await ctx.db.appSetting.findMany({
    where: { companyUuid, key: { in: [CURRENCY_SETTING_KEY, TIMEZONE_KEY, DECIMALS_KEY] } },
  });
...
export const configurationResolvers = {
  ...
  RootMutationType: {
    updateConfiguration: async (
      _: unknown,
      { request }: { request: { currency?: string; timezone?: string; decimalPlaces?: number } },
      ctx: Context,
    ) => {
